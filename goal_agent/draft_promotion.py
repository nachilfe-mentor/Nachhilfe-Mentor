...
def _is_guided_writing_draft(draft_path: Path, html: str) -> bool:
    slug = draft_path.stem.lower()
    if any(term in slug for term in GUIDED_WRITING_SLUG_TERMS):
        return True
...
def _destination_for(draft_path: Path, html: str = "") -> Path:
    stem = draft_path.stem
    stem = re.sub(r"-(practice-)?draft$", "", stem)
    if not stem:
        stem = draft_path.stem
    if _is_guided_writing_draft(draft_path, html):
        return PUBLISHED_DIR / "deutsch" / f"{stem}.html"
...
def _make_indexable(html: str, url_path: str) -> str:
    ...
    if "rel=\"canonical\"" not in html and "rel='canonical'" not in html:
        html = html.replace(
            "</head>",
            f'  <link rel="canonical" href="https://nachhilfe-mentor.de{url_path}">\n</head>',
            1,
        )
    else:
        html = re.sub(
...
def _promotion_reasons(html: str, quality: QualityResult, published_path: Path) -> list[str]:
    ...
    if not re.search(r"practice_started|practice_completed|solution_revealed|answer_checked|dataLayer\.push|practice_", html):
...
def promote_drafts(settings: Settings, limit: int | None = None) -> list[PromotionResult]:
    ...
    for draft_path in sorted(drafts_dir.glob("*.html")):
        ...
        published_path = settings.repo_root / _destination_for(draft_path, html).relative_to(REPO_ROOT)
        ...
        url_path = "/" + str(published_path.relative_to(settings.repo_root)).replace("\\", "/")
        published_html = _make_indexable(html, url_path)
