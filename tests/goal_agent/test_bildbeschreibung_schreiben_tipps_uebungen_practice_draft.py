...
SLUG = "bildbeschreibung-schreiben-tipps-uebungen"
...
def test_bildbeschreibung_tipps_spec_exists_before_draft() -> None:
    assert SPEC.exists(), f"Spec-Datei fehlt: {SPEC}"
    assert HTML.exists(), f"HTML-Draft fehlt: {HTML}"
    assert QA.exists(), f"QA-Notiz fehlt: {QA}"
    assert SPEC.stat().st_mtime <= HTML.stat().st_mtime, "Spec muss vor oder gleichzeitig mit HTML erstellt worden sein"
...
def test_bildbeschreibung_tipps_draft_contract() -> None:
    ...
    assert 'data-topic-cluster="schreibaufgaben-bildbeschreibung"' in html
...
def test_bildbeschreibung_tipps_german_copy_and_artifacts() -> None:
    html = HTML.read_text(encoding="utf-8")
    visible = _visible_text(html)
    ...
    assert "Prüfen" in visible
    assert "Musterlösung" in visible
    assert "Überarbeitung" in visible


def test_bildbeschreibung_tipps_qa_note_exists() -> None:
