"""Tests für die Prototypentheorie-Semantik-Simulation (Draft/noindex)."""
...
SPEC = ROOT / "lernmaterialien" / "entwuerfe" / "prototypentheorie-semantik-simulation-spec.md"
HTML = ROOT / "lernmaterialien" / "lernsimulationen" / "prototypentheorie-semantik-simulation.html"
QA   = ROOT / "lernmaterialien" / "entwuerfe" / "prototypentheorie-semantik-simulation-qa.md"
...
def test_spec_exists() -> None:
    ...
    assert SPEC.exists(), "Spec-Datei fehlt: prototypentheorie-semantik-simulation-spec.md"
...
def test_html_has_prediction_task() -> None:
    ...
    assert "Vorhersage" in visible or "typischst" in visible.lower() or "Welches" in visible, (
        "Vorhersage-Aufgabe nicht im sichtbaren Text"
    )
    # Auswahlmöglichkeiten für Tier-Kategorisierung
    assert "Rotkehlchen" in html, "Rotkehlchen-Option fehlt"
    assert "Adler" in html, "Adler-Option fehlt"
    assert "Pinguin" in html, "Pinguin-Option fehlt"
    assert "Fledermaus" in html, "Fledermaus-Option fehlt"
...
def test_html_has_two_step_progression() -> None:
    ...
    html = HTML.read_text(encoding="utf-8")
    assert "stage1" in html or "screen-stage1" in html or "stufe1" in html.lower(), \
        "Stufe 1 (stage1) fehlt"
    assert "stage2" in html or "screen-stage2" in html or "stufe2" in html.lower(), \
        "Stufe 2 (stage2) fehlt"
    assert "Stufe 1" in html or "Schritt 1" in html, "'Stufe 1' Text fehlt"
    assert "Stufe 2" in html or "Schritt 2" in html, "'Stufe 2' Text fehlt"


# ── Fachbegriffe im Feedback ──────────────────────────────────────────────────

def test_html_feedback_names_prototype_concepts() -> None:
    """Feedback benennt Prototypentheorie und Fachbegriffe explizit."""
    html = HTML.read_text(encoding="utf-8")
    assert "Prototyp" in html, "Begriff 'Prototyp' fehlt"
    assert "Rosch" in html, "Begriff 'Rosch' fehlt"
    assert "Typikalität" in html or "Typikalitätswert" in html or "typisch" in html.lower(), \
        "Typikalitätsbegriff fehlt"
    assert "Familienähnlichkeit" in html or "Wittgenstein" in html, \
        "Begriff 'Familienähnlichkeit'/'Wittgenstein' fehlt"


def test_html_addresses_misconception() -> None:
    """Simulation adressiert das Missverständnis 'Kategorien haben scharfe Grenzen'."""
    html = HTML.read_text(encoding="utf-8")
    assert any(phrase in html for phrase in [
        ...
        "Volksklassifikation",
        "Säugetier",
    ]), "Missverständnis-Adressierung (Kategoriengrenze) fehlt"
...
def test_html_correct_german_umlauts() -> None:
    ...
    forbidden = ["Familienaehnlichkeit", "Kategorienzugehoerigkeit", "Typikalitaet",
                 "Vorhersage".replace("e", "ae")]
...
def test_qa_note_exists() -> None:
    ...
    assert QA.exists(), "QA-Notiz fehlt: prototypentheorie-semantik-simulation-qa.md"
