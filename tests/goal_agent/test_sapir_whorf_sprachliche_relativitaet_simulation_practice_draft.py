"""Tests für die Sapir-Whorf-Hypothese-Simulation (Draft/noindex)."""
...
SPEC = ROOT / "lernmaterialien" / "entwuerfe" / "sapir-whorf-sprachliche-relativitaet-simulation-spec.md"
HTML = ROOT / "lernmaterialien" / "lernsimulationen" / "sapir-whorf-sprachliche-relativitaet-simulation.html"
QA   = ROOT / "lernmaterialien" / "entwuerfe" / "sapir-whorf-sprachliche-relativitaet-simulation-qa.md"
...
def test_html_feedback_names_concept() -> None:
    """Feedback benennt die Sapir-Whorf-Hypothese und den Wahrnehmungseffekt."""
    html = HTML.read_text(encoding="utf-8")
    assert "Sapir-Whorf" in html
    assert "Wahrnehmungseffekt" in html
    # Konzept-Tag muss im Feedback vorhanden sein
...
def test_html_correct_german_umlauts() -> None:
    ...
    assert "beeinflusst" in visible or "beeinflu" in visible
    assert "Hypothese" in visible
    # Korrekte Umlauts
    assert "Wahrnehmung" in visible
    assert "Sprachrelativität" in visible or "Sprachrelativit" in visible
...
def test_html_names_winawer_study() -> None:
    """Die Simulation benennt die Primärquelle (Winawer et al. 2007)."""
    html = HTML.read_text(encoding="utf-8")
    assert "Winawer" in html
    assert "2007" in html


def test_html_has_strong_weak_distinction() -> None:
    """Starke und schwache Hypothese müssen unterschieden werden."""
    html = HTML.read_text(encoding="utf-8")
    assert "Sprachdeterminismus" in html
    assert "Sprachrelativität" in html or "Sprachrelativit" in html
